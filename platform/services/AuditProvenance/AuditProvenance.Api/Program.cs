...
_ = builder.AddRealtimePlatformObservability("AuditProvenance.Api");

_ = builder.Services.AddDialysisPlatformC5WebApi(builder.Configuration);
_ = builder.Services.AddScoped<IAuditEventStore, EfAuditProvenanceAuditEventStore>();
...
_ = builder.Services.AddDialysisPlatformMassTransit<AuditProvenanceDbContext>(builder.Configuration);
...
_ = builder.Services.AddNpgsqlBoundedContext<AuditProvenanceDbContext>(connectionString);
...
await app.ApplyPendingMigrationsInDevelopmentAsync<AuditProvenanceDbContext>().ConfigureAwait(false);
