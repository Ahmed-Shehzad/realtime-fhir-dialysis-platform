...
_ = builder.AddRealtimePlatformObservability("QueryReadModel.Api");

_ = builder.Services.AddDialysisPlatformC5WebApi(builder.Configuration);
_ = builder.Services.AddScoped<IAuditEventStore, EfQueryReadModelAuditEventStore>();
...
_ = builder.Services.AddDialysisPlatformMassTransit<QueryReadModelDbContext>(builder.Configuration);
...
_ = builder.Services.AddNpgsqlBoundedContext<QueryReadModelDbContext>(connectionString);

_ = builder.Services.AddScoped<ISessionOverviewProjectionRepository, SessionOverviewProjectionRepository>();
_ = builder.Services.AddScoped<IAlertProjectionRepository, AlertProjectionRepository>();
...
await app.ApplyPendingMigrationsInDevelopmentAsync<QueryReadModelDbContext>().ConfigureAwait(false);
