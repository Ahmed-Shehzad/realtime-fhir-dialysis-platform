...
using ReplayRecovery.Infrastructure.Persistence;
...
_ = builder.AddRealtimePlatformObservability("ReplayRecovery.Api");

_ = builder.Services.AddDialysisPlatformC5WebApi(builder.Configuration);
_ = builder.Services.AddScoped<IAuditEventStore, EfReplayRecoveryAuditEventStore>();
...
_ = builder.Services.AddDialysisPlatformMassTransit<ReplayRecoveryDbContext>(builder.Configuration);
...
_ = builder.Services.AddNpgsqlBoundedContext<ReplayRecoveryDbContext>(connectionString);
...
await app.ApplyPendingMigrationsInDevelopmentAsync<ReplayRecoveryDbContext>().ConfigureAwait(false);
