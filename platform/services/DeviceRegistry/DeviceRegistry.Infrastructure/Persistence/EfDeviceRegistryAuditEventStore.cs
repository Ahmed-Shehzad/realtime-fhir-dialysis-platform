...
/// <summary>
/// <see cref="IAuditEventStore"/> on the scoped <see cref="DeviceRegistryDbContext"/>. Does not call <c>SaveChanges</c> —
/// commit with <see cref="IUnitOfWork"/> together with domain and outbox work.
/// </summary>
public sealed class EfDeviceRegistryAuditEventStore : IAuditEventStore
{
    private readonly DeviceRegistryDbContext _db;

    public EfDeviceRegistryAuditEventStore(DeviceRegistryDbContext db) =>
