...
public sealed class OpenApiDocumentTests : IClassFixture<MeasurementValidationApiFactory>
{
    private readonly MeasurementValidationApiFactory _factory;

    public OpenApiDocumentTests(MeasurementValidationApiFactory factory) =>
    ...
    public async Task OpenApi_v1_document_marks_authorized_operations_with_bearer_requirementAsync()
    {
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Validation.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Validation.Read").ShouldBeTrue();
    }
}
