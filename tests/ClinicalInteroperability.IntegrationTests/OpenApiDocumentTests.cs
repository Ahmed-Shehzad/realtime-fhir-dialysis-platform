...
public sealed class OpenApiDocumentTests : IClassFixture<ClinicalInteroperabilityApiFactory>
{
    private readonly ClinicalInteroperabilityApiFactory _factory;

    public OpenApiDocumentTests(ClinicalInteroperabilityApiFactory factory) =>
    ...
    public async Task OpenApi_v1_document_marks_authorized_operations_with_bearer_requirementAsync()
    {
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Interoperability.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Interoperability.Read").ShouldBeTrue();
    }
}
