...
public sealed class OpenApiDocumentTests : IClassFixture<AuditProvenanceApiFactory>
{
    private readonly AuditProvenanceApiFactory _factory;

    public OpenApiDocumentTests(AuditProvenanceApiFactory factory) =>
    ...
    public async Task OpenApi_v1_document_marks_authorized_operations_with_bearer_requirementAsync()
    {
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Audit.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Audit.Read").ShouldBeTrue();
    }
}
