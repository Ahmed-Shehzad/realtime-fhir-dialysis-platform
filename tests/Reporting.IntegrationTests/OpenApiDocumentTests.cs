...
public sealed class OpenApiDocumentTests : IClassFixture<ReportingApiFactory>
{
    private readonly ReportingApiFactory _factory;

    public OpenApiDocumentTests(ReportingApiFactory factory) =>
    ...
    [Fact]
    public async Task OpenApi_v1_document_marks_reporting_operations_with_scopesAsync()
    {
        HttpClient client = _factory.CreateClient();
        HttpResponseMessage response = await client.GetAsync(new Uri("/openapi/v1.json", UriKind.Relative));
        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Reporting.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Reporting.Read").ShouldBeTrue();
    }
}
