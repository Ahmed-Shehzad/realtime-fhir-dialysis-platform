...
public sealed class OpenApiDocumentTests : IClassFixture<TerminologyConformanceApiFactory>
{
    private readonly TerminologyConformanceApiFactory _factory;

    public OpenApiDocumentTests(TerminologyConformanceApiFactory factory) =>
    ...
    public async Task OpenApi_v1_document_marks_authorized_operations_with_bearer_requirementAsync()
    {
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Terminology.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Terminology.Read").ShouldBeTrue();
    }
}
