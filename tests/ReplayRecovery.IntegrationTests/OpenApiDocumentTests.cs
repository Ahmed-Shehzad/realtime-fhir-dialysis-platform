...
public sealed class OpenApiDocumentTests : IClassFixture<ReplayRecoveryApiFactory>
{
    private readonly ReplayRecoveryApiFactory _factory;

    public OpenApiDocumentTests(ReplayRecoveryApiFactory factory) =>
    ...
    [Fact]
    public async Task OpenApi_v1_document_marks_replay_operations_with_scopesAsync()
    {
        HttpClient client = _factory.CreateClient();
        HttpResponseMessage response = await client.GetAsync(new Uri("/openapi/v1.json", UriKind.Relative));
        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Replay.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Replay.Read").ShouldBeTrue();
    }
}
