...
public sealed class OpenApiDocumentTests : IClassFixture<AdministrationConfigurationApiFactory>
{
    private readonly AdministrationConfigurationApiFactory _factory;

    public OpenApiDocumentTests(AdministrationConfigurationApiFactory factory) =>
    ...
    [Fact]
    public async Task OpenApi_v1_document_marks_configuration_operations_with_scopesAsync()
    {
        HttpClient client = _factory.CreateClient();
        HttpResponseMessage response = await client.GetAsync(new Uri("/openapi/v1.json", UriKind.Relative));
        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Configuration.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Configuration.Read").ShouldBeTrue();
    }
}
