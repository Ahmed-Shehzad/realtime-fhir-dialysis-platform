...
public sealed class OpenApiDocumentTests : IClassFixture<ClinicalAnalyticsApiFactory>
{
    private readonly ClinicalAnalyticsApiFactory _factory;

    public OpenApiDocumentTests(ClinicalAnalyticsApiFactory factory) =>
    ...
    [Fact]
    public async Task OpenApi_v1_document_marks_clinical_analytics_operations_with_scopesAsync()
    {
        HttpClient client = _factory.CreateClient();
        HttpResponseMessage response = await client.GetAsync(new Uri("/openapi/v1.json", UriKind.Relative));
        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Analytics.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Analytics.Read").ShouldBeTrue();
    }
}
