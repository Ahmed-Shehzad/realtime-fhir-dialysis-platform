...
public sealed class OpenApiDocumentTests : IClassFixture<RealtimeSurveillanceApiFactory>
{
    private readonly RealtimeSurveillanceApiFactory _factory;

    public OpenApiDocumentTests(RealtimeSurveillanceApiFactory factory) =>
    ...
    public async Task OpenApi_v1_document_marks_authorized_operations_with_bearer_requirementAsync()
    {
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Surveillance.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Surveillance.Read").ShouldBeTrue();
    }
}
