...
public sealed class OpenApiDocumentTests : IClassFixture<SignalConditioningApiFactory>
{
    private readonly SignalConditioningApiFactory _factory;

    public OpenApiDocumentTests(SignalConditioningApiFactory factory) =>
    ...
    public async Task OpenApi_v1_document_marks_authorized_operations_with_bearer_requirementAsync()
    {
        ...
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Conditioning.Write").ShouldBeTrue();
        OpenApiBearerSecurityScan.AnyBearerRequirementListsScope(paths, "Dialysis.Conditioning.Read").ShouldBeTrue();
    }
}
